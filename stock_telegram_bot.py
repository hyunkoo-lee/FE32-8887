...
def generate_open_close_chart(stock_results, chart_img_path):
    """시가 vs 종가 등락률 도식화 바 차트 이미지 생성"""
    font_path = os.path.join(tempfile.gettempdir(), "NanumGothic.ttf")
    ...
    names = [r['name'][:14] for r in valid_items]
    pcts = [r['data']['change_pct'] for r in valid_items]
    colors_list = ['#DC2626' if p > 0 else ('#2563EB' if p < 0 else '#64748B') for p in pcts]

    # 가로 바 차트 생성
    fig, ax = plt.subplots(figsize=(10.5, max(3.5, len(valid_items) * 0.45 + 1.2)))
    bars = ax.barh(names, pcts, color=colors_list, height=0.55)

    ax.axvline(0, color='#94A3B8', linewidth=1, linestyle='--')
    ax.set_xlabel('시가 대비 종가 등락률 (%)', fontsize=10, fontweight='bold', labelpad=10)
    ax.set_title('📊 종목별 시가 ➔ 종가 변동 도식화 차트', fontsize=13, fontweight='bold', pad=12)
    ...
    for bar, item in zip(bars, valid_items):
        ...
        ax.text(width + offset, bar.get_y() + bar.get_height()/2, price_text, 
                va='center', ha=ha, fontsize=8, color='#1E293B')

    plt.tight_layout()
    plt.savefig(chart_img_path, dpi=200, bbox_inches='tight')
    plt.close()
    return chart_img_path

def create_stock_pdf_report(stock_results, latest_date, pdf_filepath):
    """ReportLab을 사용하여 한 행에 모든 시세 정보가 출력되고 차트가 포함된 가로형 PDF 생성"""
    ...
    # 순서: 종목코드(70), 종목명(165), 시가(75), 종가(75), 수익률(75), 당일 변동 범위(125), 메모 및 보유전략(197)
    col_widths = [70, 165, 75, 75, 75, 125, 197]
    ...
    # 시가 ➔ 종가 도식화 차트 생성 및 추가
    chart_filepath = os.path.join(tempfile.gettempdir(), "stock_open_close_chart.png")
    if generate_open_close_chart(stock_results, chart_filepath) and os.path.exists(chart_filepath):
        elements.append(Spacer(1, 15))
        elements.append(Paragraph("<b>📈 종목별 시가 ➔ 종가 변동 도식화 그래프</b>", title_style))
        elements.append(Spacer(1, 5))
        chart_h = min(320, len(stock_results) * 22 + 80)
        elements.append(Image(chart_filepath, width=780, height=chart_h))
...
def format_telegram_message(stock_results):
    ...
    msg += f"📎 <i>한눈에 들어오는 전체 종합 시세 표 및 도식화 그래프는 아래 첨부된 PDF 문서로 확인하실 수 있습니다.</i>\n\n"
...
def main():
    ...
    # 1. PDF 리포트 파일 생성 (시가/종가 도식화 차트 포함)
    ...
    doc_res = send_telegram_document(
        ...
        pdf_filepath, 
        pdf_filename, 
        caption=f"<b>📄 주식 시세 요약 표 및 도식화 그래프 PDF 문서 ({latest_date})</b>"
    )
