...
# 매년 변경 가능한 보유 계좌 구글 시트 URL (환경변수 설정 지원)
HOLDINGS_SHEET_URL = os.getenv("HOLDINGS_SHEET_URL") or "https://docs.google.com/spreadsheets/d/1MsOdBxl6fG9uL9K_SwqjYkjlcqvBm9qI0ZJ5ITTz-0Q/edit?gid=312482996#gid=312482996"
...
def extract_sheet_id(url_or_id):
    """
    [구글 시트 ID 추출 함수]
    사용자가 입력한 전체 구글 시트 URL 또는 순수 시트 ID 문자열에서 구글 시트 식별자(Spreadsheet ID)만 추출합니다.
    """
    if not url_or_id:
        return None
    if "/d/" in url_or_id:
...
def get_holdings_gviz_csv_url(url_or_id):
    """
    [보유 계좌 구글 시트 URL 자동 변환 함수]
    사용자가 매년 새로 입력하는 구글 시트 전체 URL (https://docs.google.com/spreadsheets/d/.../edit?gid=...) 또는
    시트 식별자에서 Sheet ID와 GID를 추출하여 GViz CSV 내보내기 URL로 자동 변환합니다.
    """
    if not url_or_id:
        url_or_id = HOLDINGS_SHEET_URL
    
    sheet_id = extract_sheet_id(url_or_id)
    gid = "312482996"
    if "gid=" in url_or_id:
        try:
            gid_part = url_or_id.split("gid=")[1]
            gid = gid_part.split("#")[0].split("&")[0].split("/")[0]
        except Exception:
            pass
...
def normalize_ticker_symbol(symbol):
    ...
    - 국내 6자리 종목코드(신종목코드 포함) 뒤에 '.KS' 접미사 부착
...
def normalize_display_symbol(symbol):
    """
    [표시용 종목코드 정돈 함수]
    텔레그램 메시지 및 PDF 요약표에 출력하기 위해 '.KS', '.KQ'를 제거한 6자리 순수 종목코드를 반환합니다.
    """
...
def fetch_naver_stock_name(symbol_or_code):
    """
    [네이버 증권 한글 종목명 조회 함수]
    네이버 증권 API를 호출하여 국내 주식/ETF(신종목코드 포함) 및 해외 주식의 공식 한글 종목명을 가져옵니다.
    """
...
def fetch_portfolio_holdings_from_google_sheet(sheet_url_or_id=None):
    """
    [보유 계좌 구글 시트 파싱 함수]
    매년 변경 가능한 보유 계좌 시트 URL(HOLDINGS_SHEET_URL)을 가져와
    소유주(A열: 소유), 계좌(B열), 종목명(E열: 종목), 수량(G열), 평가금액(J열), 종목코드(N열)를 파싱합니다.
    """
    target_url = get_holdings_gviz_csv_url(sheet_url_or_id or HOLDINGS_SHEET_URL)
    try:
        print(f"📖 보유 계좌 구글 시트 조회 중... ({target_url})")
        ...
        for r in reader[1:]:
            ...
            if owner in ['합계', '', '소유'] or stock_name in ['종목', '']:
                continue
...
def get_stock_data(symbol):
    """
    [주가 시세 및 뉴스 수집 함수]
    yfinance 라이브러리를 이용해 시가, 고가, 저가, 종가, 등락률(%), 통화 단위 및 최근 뉴스 수집
    """
    try:
...
def generate_owner_evaluation_chart(holdings, chart_img_path, stock_results=None):
    """
    ...
    if stock_results:
        for r in stock_results:
            ...
            if d:
                ...
                change_map[r['symbol']] = pct
                if 'display_symbol' in r:
                    change_map[r['display_symbol']] = pct
                change_map[normalize_ticker_symbol(r['symbol'])] = pct
        ...
        for item in items:
            ...
            if change_pct is None and norm_ticker:
                ...
                if sd:
                    ...
                    change_map[ticker] = change_pct
                    change_map[norm_ticker] = change_pct

            is_decreased = (change_pct is not None and change_pct < 0)
            decrease_flags.append((is_decreased, change_pct or 0.0))
    ...
    # 막대 상단 수치 표기 (전일 대비 하락 시 빨간색 ▼ 표기)
    for bar, (is_dec, chg) in zip(bars, decrease_flags):
        h = bar.get_height()
        if is_dec:
            lbl = f"₩{h:,.0f}만  ▼"
            txt_color = '#DC2626' # 빨간색 체크 강조
        else:
            lbl = f"₩{h:,.0f}만"
            txt_color = '#0F172A'
...
def create_stock_pdf_report(stock_results, holdings, latest_date, pdf_filepath):
    ...
    cell_p4_num_right_red = ParagraphStyle(
        'CellP4NumRightRed',
        parent=styles['Normal'],
        fontName=bold_font_name,
        fontSize=8.5,
    ...
    cell_p4_num_right_blue = ParagraphStyle(
        'CellP4NumRightBlue',
        parent=styles['Normal'],
        fontName=bold_font_name,
        fontSize=8.5,
    ...
    cell_p4_num_right_black = ParagraphStyle(
        'CellP4NumRightBlack',
        parent=styles['Normal'],
        fontName=font_name,
        fontSize=8.5,
    ...
    cell_p4_pct_center_red = ParagraphStyle(
        'CellP4PctCenterRed',
        parent=styles['Normal'],
        fontName=bold_font_name,
        fontSize=8.5,
    ...
    cell_p4_pct_center_blue = ParagraphStyle(
        'CellP4PctCenterBlue',
        parent=styles['Normal'],
        fontName=bold_font_name,
        fontSize=8.5,
    ...
    cell_p4_pct_center_black = ParagraphStyle(
        'CellP4PctCenterBlack',
        parent=styles['Normal'],
        fontName=font_name,
        fontSize=8.5,
        leading=11,
    ...
    # --- Page 3: 소유주별 / 종목별 전체 자산 평가금 풀페이지 도식화 차트 (하락 종목: 빨간색 ▼ 체크 표기) ---
    if holdings:
        owner_chart_filepath = os.path.join(tempfile.gettempdir(), "owner_evaluation_chart.png")
        if generate_owner_evaluation_chart(holdings, owner_chart_filepath, stock_results) and os.path.exists(owner_chart_filepath):
            elements.append(PageBreak())
            elements.append(Paragraph("<b>💼 [소유주별 / 종목별 전체 자산 평가금 현황 도식화 차트]</b>", title_style))
            elements.append(Spacer(1, 8))
            elements.append(Image(owner_chart_filepath, width=790, height=485))

    # --- Page 4: 매월 1일~말일 일별 자산 평가금 관리 표 ---
    ...
    def make_column_data(rows):
        data = [tbl_hdr]
        for r in rows:
            date_p = Paragraph(r['date_day'], cell_p4_date)

            if r['color_type'] == 'red':
                val_p = Paragraph(r['val_str'], cell_p4_num_right_red)
                chg_p = Paragraph(r['change_str'], cell_p4_num_right_red)
                pct_p = Paragraph(r['change_pct_str'], cell_p4_pct_center_red)
            elif r['color_type'] == 'blue':
                val_p = Paragraph(r['val_str'], cell_p4_num_right_blue)
                chg_p = Paragraph(r['change_str'], cell_p4_num_right_blue)
                pct_p = Paragraph(r['change_pct_str'], cell_p4_pct_center_blue)
            else:
                val_p = Paragraph(r['val_str'], cell_p4_num_right_black)
                chg_p = Paragraph(r['change_str'], cell_p4_num_right_black)
                pct_p = Paragraph(r['change_pct_str'], cell_p4_pct_center_black)
...
def format_telegram_message(stock_results):
    ...
    msg += f"📎 <i>시세 표, 등락률 차트, 소유주별 전체 평가금 차트 및 월간 일별 평가금 관리 표는 첨부된 4페이지 PDF 문서로 확인하실 수 있습니다.</i>\n\n"
...
def main():
    ...
    target_tickers = []
    if GOOGLE_SHEET_URL:
        print(f"📖 관심 종목 구글 시트 조회 중... ({GOOGLE_SHEET_URL})")
    ...
    print("💼 보유 계좌 구글 시트 데이터 파싱 중...")
    holdings = fetch_portfolio_holdings_from_google_sheet(HOLDINGS_SHEET_URL)
    ...
    doc_res = send_telegram_document(
        ...
        pdf_filepath, 
        pdf_filename, 
        caption=f"<b>📄 주식 시세 요약, 등락률 차트, 소유주별 전체 평가금 차트 및 월간 관리 표 4페이지 PDF 문서 ({latest_date})</b>"
    )
